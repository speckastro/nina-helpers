...
namespace SpeckSequenceHelpers.Core.Tests {
    ...
    public class SkyMedianGateTests {
        ...
        public void Constructor_NonFiniteThresholds_Throw() {
            Assert.Throws<ArgumentException>(() => new SkyMedianGate(double.NaN, 5000, GateDirection.Brightening));
            Assert.Throws<ArgumentException>(() => new SkyMedianGate(1000, double.NaN, GateDirection.Brightening));
            Assert.Throws<ArgumentException>(() => new SkyMedianGate(1000, double.PositiveInfinity, GateDirection.Dimming));
        }
    }
}
